...
#            print("into train后GPU1显存变至{}".format(after - before))
    ...
    def eval_step(n_samples, sample_steps):
        ...
        def sample_fn(_n_samples, return_caption=False, return_clipScore=False, ClipSocre_model=None, config=None):
            ...
            _image_latent_sqs,_context,_token_mask,_batch_img_ori = next(context_generator) # 依次返回：正常image经过autoencoder的结果、janus提取的text image embedding、janus提取后的mask、正常image
        ...
        with tempfile.TemporaryDirectory() as temp_path:
            ...
            _fid = 0
            if accelerator.is_main_process:
                _fid = calculate_fid_given_paths(("/storage/v-jinpewang/lab_folder/qisheng_data/text_image_testset_noextract/imgs", path))
...
def main(argv):
    ...
    config.workdir = FLAGS.workdir or os.path.join('/storage/v-jinpewang/lab_folder/qisheng_data/workdir', config.config_name, config.hparams)
