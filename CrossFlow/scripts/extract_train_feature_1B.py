...
def main(bz = 4):
    ...
    json_path = '/storage/qisheng_azure/CrossFlow/text_image_dataset_small/img_img_pair.jsonl'
    image_root_path = '/storage/qisheng_azure/CrossFlow/text_image_dataset_small/imgs'
    render_root_path = '/storage/qisheng_azure/CrossFlow/text_image_dataset_small/render_imgs'
    ...
    save_dir = f'/storage/qisheng_azure/CrossFlow/text_image_dataset_small_1B/feature'
    ...
    for i, sample in enumerate(tqdm(dicts_list)):
        ...
        layers_hidden_states = outputs.hidden_states  #Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of torch.FloatTensor of shape (batch_size, generated_length, hidden_size).
        ...
        padded_samples = [padding_sample for _ in range(num_padding_samples)]  #每一层回答离max len差的token用全0tensor补齐
        ...
        squeezed_tensor = stacked_tensor.squeeze(dim=2)  # 形状变为 [1, 512, 2048]
        final_tensor = squeezed_tensor.squeeze(dim=0)  # 形状变为 [512, 2048]
        ...
        if len(batch_name) == bz or i == len(dicts_list) - 1:
            ...
            for mt_256, mt_512, te_t, tm_t, bn in zip(moments_256, moments_512, token_embedding_janus, mask_janus, batch_name):
                ...
                assert te_t.shape == (512, 2048)  #modified
