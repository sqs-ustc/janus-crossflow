...
class TransEncoder(nn.Module):
    def __init__(self, d_model, N, num_token, head_num, d_ff, latten_size, down_sample_block=3, dropout=0.1, last_norm=True):
        ...
        if d_model==4096:
            ...
            print("TransEncoder compressor changed dim! Janus7B")
            d_model = 1024
        elif d_model==2048:
            # for Janus-Pro-1B, first use MLP to compress into 1024
            self.compressor = Compressor(input_dim=d_model, tar_dim=768)
            print("TransEncoder compressor changed dim! Janus1B")
            d_model = 768
        else:
            self.compressor = None
            print("TransEncoder no compressor!")
